...
ROI_CENTROIDS_PATH = (
    DATA_METADATA / "roi_centroids.json"
)  # ROI 3D centroids for visualization
...
LOBE_MAPPING = {
    ...
    4: _idx(
        [33, 34, 35, 36, 37, 38, 151, 152, 153, 154, 155, 156]
    ),  # Cingulate + ACC subdivisions
    ...
    10: _idx(
        list(range(95, 121)) + list(range(157, 167))
...
# 12-lobe test AUC 0.8694 outperforms 11-lobe by +5.3% (controlled comparison)
...
SPATIAL_MIN_REQUIRED_REGIONS = (
    9  # relaxed gate; final golden filter enforces complete 12-region subjects
)
...
if USE_11_LOBES:
    ...
    NETWORK_TO_LOBES = {
        k: [lb for lb in v if lb < 11] for k, v in NETWORK_TO_LOBES.items()
    }
