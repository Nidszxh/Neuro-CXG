...
def main():
    ...
    for split in splits:
        ...
        if not img_dir.exists():
            logger.warning(f"⚠️ Warning: Split folder {split} is missing images. Skipping.")
            continue
            
        lbl_dir.mkdir(parents=True, exist_ok=True)
        img_files = [f for f in os.listdir(img_dir) if f.endswith('.png')]
        
        logger.info(f"🏷️  Annotating {split} split ({len(img_files)} images)...")
    ...
    logger.info(f"✅ Annotation Complete. Labels synced with split images.")
