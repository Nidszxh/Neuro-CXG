    ...
    for key, slist in sorted(key_to_sites.items(), key=lambda x: -len(x[1])):
        ...
        if len(slist) == 1:
            ...
            label = next(
                (
                    f"{k[0]}_{k[1]}"
                    for k, sl in key_to_sites.items()
...
def generate_site_stratified_folds(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if len(train_df) == 0:
        logger.warning(
            "No training subjects found; skipping site-stratified CV generation."
        )
        return df
...
def _resolve_source_ts_dir() -> Path:
    ...
    logger.info(
        "No source time-series directory found; defaulting to %s", DATA_TIME_SERIES
    )
...
def run_stratified_split():
    ...
    if MASTER_MANIFEST.exists():
        try:
            ...
            if "split" in manifest_df.columns:
                ...
                logger.info(
                    "Found 'split' column in manifest — using pre-assigned splits"
                )
                manifest_id_col = (
                    "FILE_ID" if "FILE_ID" in manifest_df.columns else "subject_id"
                )
                if manifest_id_col not in manifest_df.columns:
                    logger.warning(
                        "Manifest has split column but no FILE_ID/subject_id; ignoring."
                    )
                    manifest_has_splits = False
                else:
                    manifest_df = manifest_df.copy()
                    manifest_df["FILE_ID"] = (
                        manifest_df[manifest_id_col].astype(str).str.strip()
                    )
                    df = df.merge(
                        manifest_df[["FILE_ID", "split"]], on="FILE_ID", how="left"
                    )
    ...
    if not manifest_has_splits:
        ...
        if len(valid_groups) == 0:
            raise ValueError(
                "Cannot proceed with stratification: no valid groups remain."
            )
        ...
        logger.info(
            f"Filtered to {len(df)} subjects using stratification groups (min {min_group_size})."
        )
    ...
    if manifest_has_splits:
        ...
        train_df, rem_df = train_test_split(
            ...
            stratify=df[["DX_GROUP", "SITE_ID"]],
            random_state=42,
        )
        val_df, test_df = train_test_split(
            rem_df,
            train_size=0.5,
            stratify=rem_df[["DX_GROUP", "SITE_ID"]],
            random_state=42,
        )
    ...
    if "cv_fold" not in df.columns or df["cv_fold"].isna().any():
        ...
        if len(train_idx_overall) > 0:
            strat_col = (
                df.loc[train_idx_overall, "DX_GROUP"].astype(str)
                + "_"
                + df.loc[train_idx_overall, "SITE_ID"].astype(str)
            )
            if (strat_col.value_counts() < 2).any():
                logger.warning(
                    "Falling back to DX_GROUP-only stratification for CV folds."
                )
                strat_col = df.loc[train_idx_overall, "DX_GROUP"].astype(str)
            for fold, (_, val_idx) in enumerate(
                skf.split(train_idx_overall, strat_col)
            ):
                actual_val_indices = [train_idx_overall[i] for i in val_idx]
                df.loc[actual_val_indices, "cv_fold"] = fold
            logger.info(
                f"Generated 5-fold StratifiedKFold CV for {len(train_idx_overall)} training subjects."
            )
