...
## DD-011: Optimized GNN hyperparameters for publication performance (April 2026, updated April 24)

- Decision: use site conditioning, demographic conditioning, and anatomical pooling for publication-quality results.
- Key settings (as of April 24 investigation):
  - `GNN_USE_SITE_EMBEDDING = True` — adds site-aware conditioning
  - `GNN_USE_DEMOGRAPHICS = True` — adds demographic context
  - `GNN_GRL_ALPHA = 0.10` and `GNN_GRL_ALPHA_MAX = 0.10` — conservative adversarial debiasing
    - **Note**: GRL_ALPHA=1.0 was originally set here but the April 24 investigation showed it degrades test AUC
      in the main pipeline (test 0.8498 vs 0.8753). Alpha=0.10 is the validated production setting.
      GRL=1.0 remains effective in the isolated ablation script but not in the full pipeline context.
...
- Results (run pipeline_20260424_191537):
  ...
  - Test AUC: 0.8753, 95% CI [0.8521, 0.8985] (was 0.7325)
  - Test F1: 0.8121 (was 0.6338)
- Source of truth:
  - `src/core/hyperparams.py`
  - `docs/CHANGELOG.md` (April 24 entry)
