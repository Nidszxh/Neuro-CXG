...
## Complete End-to-End Data Flow (20 Stages — March 1, 2026)
...
└──────────────┬───────────────────┘  - Dropout 0.45, L2 reg 1e-4, gradient clip 1.0
          ...
          ✓ best_model_fold3.pt
                ✓ best_model_fold4.pt
                         │
                         ▼
            📊 STAGE 17–20: POST-TRAINING ANALYSIS (Phase 9, March 1, 2026)
            ├─ Stage 17: Visualizations — causal graph plots, feature heatmaps
            ├─ Stage 18: Evaluation — bootstrap 95% CI, permutation test, baselines
...
pipeline_checks.py                  ✓ YES     Stages 6/7/15              ✓ Consolidated
...
run_evaluation.py                    ✓ YES     Stage 18 (post-training)   ✓ Phase 9
run_explainability.py                ✓ YES     Stage 19 (post-training)   ✓ Phase 9
run_result_analysis.py               ✓ YES     Stage 20 (post-training)   ✓ Phase 9

* pipeline_checks.py consolidates integrity_check.py + integrity_check2.py + pipeline_diagnostics.py
     (Deleted: integrity_check.py, integrity_check2.py, pipeline_diagnostics.py)
...
```
src.run_pipeline (ORCHESTRATOR — 20 stages)
├── src.data.abide_download
│   └─→ Outputs: phenotype CSV, PNG images (7 z-slices/subject)
├── src.data.split
│   └─→ Outputs: train/val/test splits (70/15/15, 2D stratification)
├── src.utils.manifestor
│   └─→ Outputs: master_manifest.csv
├── src.validation.atlas_validator
│   └─→ Validates: atlas files (164–170 ROI range)
├── src.validation.pipeline_checks ✨ UPDATED (March 1, 2026)
...
│   └─→ generate_health_report(): Comprehensive health check
...
│   └─→ Outputs: node_attributes_temporal.csv (20 features × 170 ROIs)
├── src.features.fold_safe_harmonization
│   └─→ Outputs: node_attributes_harmonized.csv (28 features × 12 regions)
...
│
├── [POST-TRAINING ANALYSIS — Phase 9] (Stages 17–20)
│   ├── src.run_pipeline → src.analysis.visualizations  (Stage 17)
│   ├── src.run_evaluation.py  (Stage 18)
│   │   └─→ Bootstrap 95% CI, permutation test, SVM/RF/MLP baselines
│   ├── src.run_explainability.py  (Stage 19)
│   │   └─→ Captum integrated gradients, node/edge importance
│   └── src.run_result_analysis.py  (Stage 20)
│       └─→ Per-subject predictions, misclassification, site effects, calibration
│
└── [VALIDATION SUITE] src.validation.* (4 modules)
    ├── atlas_validator.py: Atlas file validation
    ├── dev_audit.py: Deep validation checks (merged from code_audit + feature_diagnostics, Feb 28, 2026)
    ├── diagnose_features.py: Per-feature-group stats, z-score audit, graph structure diagnostics (NEW, March 1, 2026)
    └── pipeline_checks.py: Post-download/pre-GNN/quality checks + health reports (4 functions)
...
Legend:
  ...
  ✨ UPDATED: Recently updated (March 1, 2026)
```
