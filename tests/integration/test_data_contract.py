...
class TestGraphDataContract:
    ...
    def test_adj_shape(self):
        ...
        graph = torch.load(find_sample_graph(), weights_only=True)
        adj = graph["adj"]

        assert adj.ndim == 2, f"adj should be 2D, got {adj.ndim}"
        assert adj.shape[0] == adj.shape[1], "adj should be square"
        assert adj.shape[0] == NUM_LOBES, f"Expected {NUM_LOBES}x{NUM_LOBES} adj ({NUM_LOBES} lobes), got {adj.shape}"
    ...
    def test_internal_features_shape(self):
        ...
        assert internal.shape[0] == NUM_LOBES, f"Expected {NUM_LOBES} lobes, got {internal.shape[0]}"
